...
PROJECTS_DATABASE = {
    ...
    "SENSI": {
        "url": "https://docs.google.com/spreadsheets/d/e/2PACX-1vSLDSBkzA1ZpD1qCRFjl4TiNWldYobalUdgwADyljTFkWMJrvVXajgFxegKWDr2SA-UcuAc8mGonW36/pub?gid=645484835&single=true&output=csv", # تم التأكد من GID
        "gov_pct": 2.0,
        "admin_fees": 625,
        "res_fee": 50000
    },
...
def create_sales_offer_pdf(unit_data, financials, schedule, layout_url, plan_name, project_name):
    ...
    try: 
        ...
        pdf.image(BytesIO(res_logo.content), x=10, y=8, w=35)
    except: pass
...
if df_inventory is not None:
    ...
    # تنظيف الأرقام من الفواصل
    def clean_val(v): return float(str(v).replace(',', '').strip()) if v and str(v).lower() != 'nan' else 0.0
    
    u_price = clean_val(unit_data.get('Original Price (AED)', '0'))
    total_disc_pct = ALL_PLANS[selected_plan]['disc'] + extra_disc
    selling_price = (u_price * (1 - total_disc_pct/100)) + clean_val(unit_data.get('parking', '0'))
    ...
    if df_photos is not None:
        try:
            # تنظيف مفتاح المشروع للبحث (حل مشكلة SENSI)
            p_key = selected_project.split()[0].upper().strip()
            ...
            # البحث مع التجاهل التام للمسافات الزائدة في SENSI
            match = df_photos[
    ...
    with c2:
        ...
        st.download_button("Download PDF", data=pdf_bytes, file_name=f"Offer_{unit_id}.pdf", use_container_width=True, type="primary")
